...
export default function FeedbackProfissionalPage() {
  ...
  useEffect(() => {
    ...
    ]).then(([p, o]) => { setProfissionais(p); setOcorridos(o) })
  }, [selected])
  ...
  async function saveProf() {
    ...
    if (res.ok) { const u = await res.json(); setProfissionais(prev => prev.map(p => p.id === u.id ? u : p)); setEditingProf(null); toast.success('Salvo!') }
  }

  async function deleteProf(id: string) {
    if (!confirm('Excluir profissional?')) return
    await fetch('/api/feedback-prof/profissionais', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) })
  ...
  async function saveOcorr() {
    ...
    if (res.ok) { const u = await res.json(); setOcorridos(prev => prev.map(o => o.id === u.id ? u : o)); setEditingOcorr(null); toast.success('Salvo!') }
  }

  async function deleteOcorr(id: string) {
    if (!confirm('Excluir ocorrido?')) return
    await fetch('/api/feedback-prof/ocorridos', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) })
  ...
  if (loading) return <div className="nodri-salon-bg min-h-screen flex items-center justify-center"><div className="text-nodri-t2 text-sm">Carregando...</div></div>
    ...
    <div className="nodri-salon-bg min-h-screen">
      ...
      <nav className="bg-nodri-surface border-b border-nodri-border px-5 py-3 flex items-center gap-3 sticky top-0 z-50">
        ...
        <div className="ml-auto flex items-center gap-2">
          {selected && (
            <button onClick={() => router.push(`/salon/feedback-profissional/resultados/${selected.id}`)}
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-1.5">
                  <button onClick={toggleAtivo} className="flex items-center gap-1 px-2.5 py-1 rounded-md text-[10px] border" style={{ borderColor: 'rgba(255,255,255,.1)', color: '#94a3b8' }}>
                    {selected.ativo ? <><Eye size={10}/> Desativar</> : <><Eye size={10}/> Ativar</>}
                  </button>
              ...
              <div className="flex gap-0">
                {(['profissionais', 'ocorridos', 'link'] as const).map(t => (
                  <button key={t} onClick={() => setTab(t)}
                    className={`px-4 py-2 text-[11px] font-medium border-b-2 transition-all capitalize ${tab === t ? 'border-nodri-cyan text-nodri-cyan' : 'border-transparent text-nodri-t3 hover:text-nodri-t1'}`}>
                    {t === 'profissionais' ? <><Users size={11} className="inline mr-1"/>Profissionais ({profissionais.length})</> :
                     t === 'ocorridos' ? <><ClipboardList size={11} className="inline mr-1"/>Ocorridos ({ocorridos.length})</> :
                     '🔗 Link do Formulário'}
                  </button>
                ))}
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-5">
              ...
              {tab === 'profissionais' && (
                <div className="max-w-xl space-y-3">
                  ...
                  <div className="flex gap-2">
                    <input value={novoProf} onChange={e => setNovoProf(e.target.value)}
                      onKeyDown={e => e.key === 'Enter' && addProf()}
                    ...
                    <button onClick={addProf} disabled={saving || !novoProf.trim()}
                  ...
                  <div className="space-y-1.5">
                    {profissionais.map(p => (
                      <div key={p.id} className="flex items-center gap-2 p-3 rounded-xl border" style={{ background: '#0d1117', borderColor: 'rgba(255,255,255,.06)' }}>
                        {editingProf?.id === p.id ? (
                          <>
                            <input value={editingProf.nome} onChange={e => setEditingProf({ ...editingProf, nome: e.target.value })}
                            ...
                            <button onClick={() => setEditingProf(null)} className="w-7 h-7 rounded-md flex items-center justify-center text-nodri-t3 hover:bg-nodri-surface"><X size={12}/></button>
                          </>
                        ) : (
                          <>
              ...
              {tab === 'ocorridos' && (
                <div className="max-w-xl space-y-3">
                  <div className="flex gap-2">
                    <input value={novoOcorr} onChange={e => setNovoOcorr(e.target.value)}
                      onKeyDown={e => e.key === 'Enter' && addOcorr()}
                      placeholder="Descrição do ocorrido..."
                      className="flex-1 bg-nodri-card border border-nodri-border rounded-lg px-3 py-2 text-[12px] text-nodri-t1 placeholder-nodri-t3 outline-none focus:border-nodri-cyan/40 uppercase" />
                    <button onClick={addOcorr} disabled={saving || !novoOcorr.trim()}
                      className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-[11px] font-bold disabled:opacity-50"
                      style={{ background: 'rgba(34,197,94,.15)', color: '#4ade80', border: '1px solid rgba(34,197,94,.3)' }}>
                      <Plus size={13}/> Adicionar
                    </button>
                    ...
                    {ocorridos.map(o => (
                      <div key={o.id} className="flex items-center gap-2 p-3 rounded-xl border" style={{ background: '#0d1117', borderColor: 'rgba(255,255,255,.06)' }}>
                        {editingOcorr?.id === o.id ? (
                          <>
                            <input value={editingOcorr.descricao} onChange={e => setEditingOcorr({ ...editingOcorr, descricao: e.target.value })}
                              className="flex-1 bg-nodri-card border border-nodri-cyan/30 rounded-lg px-2 py-1 text-[12px] text-nodri-t1 outline-none uppercase" />
              ...
              {tab === 'link' && (
                <div className="max-w-lg space-y-4">
                  <div className="p-5 rounded-2xl border" style={{ background: '#0d1117', borderColor: 'rgba(139,92,246,.25)' }}>
                    <div className="text-[11px] font-bold text-purple-400 mb-3 flex items-center gap-2">
                      🔗 Link do Formulário para Registro
                    </div>
                    <p className="text-[11px] text-nodri-t3 mb-4">
                      Compartilhe com coordenadores, gerentes ou líderes para registrar ocorrências dos profissionais. O formulário exibe a lista de profissionais e ocorridos cadastrados.
                    </p>
                    <div className="flex items-center gap-2 p-3 rounded-xl border mb-3" style={{ background: '#060a0f', borderColor: 'rgba(255,255,255,.08)' }}>
                      ...
                      <button onClick={() => { navigator.clipboard.writeText(linkForm); toast.success('Link copiado!') }}
