...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await supabaseAdmin
    .from('salao_usuarios')
    .insert({ salao_id: salaoId, nome: nome || null, usuario: String(usuario).trim().toLowerCase(), senha_hash, papel: papel || null, permissoes: Array.isArray(permissoes) ? permissoes : [], ativo: true })
