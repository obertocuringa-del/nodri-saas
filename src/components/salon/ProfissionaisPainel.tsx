        ...
        <main style={{ flex: 1, padding: isMobile ? '12px' : '24px', overflowY: 'auto', background: '#f0eeea' }}>
          ...
          {secao === 'cnpj' && (
            <div style={{ maxWidth: '1000px' }}>
              ...
              {cnpjSub === 'contratacao' ? (
                <ProcessoContratacao chave="processo_contratacao_pj" modelo={PJ_CONTRATACAO} titulo="Processo de Contratação (PJ/MEI)" comCarta={false} pessoas={profissionais.map(p => ({ nome: p.apelido || p.nome_completo || '—', telefone: (p as any).telefone || (() => { try { return JSON.parse((p as any).contato_responsavel || '{}').tel || '' } catch { return '' } })() }))} />
