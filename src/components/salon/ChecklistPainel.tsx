...
export default function ChecklistPainel({ categoriaFixa = '', embutido = false }: { categoriaFixa?: string; embutido?: boolean } = {}) {
  ...
  const carregar = useCallback(async () => {
    try {
      ...
      setDoc(docNovo)
      // Categoria alvo: prop (dentro do setor) ou ?cat= (link direto)
      try {
  ...
  return (
    <div className={embutido ? '' : 'nodri-salon-bg'} style={embutido ? undefined : { minHeight: '100vh', background: temAlerta ? '#fbe9e9' : undefined, transition: 'background .4s' }}>
      ...
      <div style={{ margin: '0 auto', padding: embutido ? 0 : (isMobile ? 16 : '16px 28px') }}>
        {loading ? <div style={{ display: 'flex', justifyContent: 'center', padding: 50 }}><Loader2 size={26} className="animate-spin" style={{ color: '#5b4fcf' }} /></div> : (<>
          ...
          {/* Abas de categorias — escondidas quando o setor já fixa a categoria */}
          {categoriaFixa ? null : isMobile ? (
