                ...
                <tbody>
                  {saloes.map(salao => (
                    <tr key={salao.id} className="border-b border-nodri-border/50 hover:bg-white/[0.01] transition-colors">
                      ...
                      <td className="px-4 py-3 text-[11px]">{salao.status === 'trial' ? (() => { const criado = new Date(salao.criado_em); const expira = new Date(criado); expira.setDate(expira.getDate() + 7); const hoje = new Date(); const dias = Math.ceil((expira.getTime() - hoje.getTime()) / (1000 * 60 * 60 * 24)); if (dias <= 0) return <span className="text-nodri-red font-semibold">🔴 Expirado</span>; if (dias <= 2) return <span className="text-nodri-red font-semibold">⚠️ {dias}d restante</span>; if (dias <= 4) return <span className="text-nodri-amber font-semibold">⏳ {dias}d restantes</span>; return <span className="text-nodri-green font-semibold">⏳ {dias}d restantes</span>; })() : <span className="text-nodri-t2">{salao.licenca_vencimento ? new Date(salao.licenca_vencimento).toLocaleDateString('pt-BR') : '—'}</span>}</td>
