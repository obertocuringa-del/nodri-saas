...
//
// Os artigos padrão moram aqui, e não numa rota, por uma limitação real do
// Next: arquivo de rota só pode exportar os handlers HTTP. Enquanto isso ficou
// dentro de route.ts, a lista não podia ser reaproveitada por mais ninguém —
// e era justamente isso que deixava a sincronia dependendo de um clique manual
// no painel master.
//
// Fonte da verdade dos artigos padrão. Quem serve a tela é o banco; este
// arquivo é o que o banco recebe quando falta alguma coisa.
...
// Insere os artigos do catálogo que ainda não existem no banco.
//
// Compara por título e nunca sobrescreve: artigo editado pelo painel continua
// como está, e artigo ocultado (ativo = false) continua ocultado, porque a
// linha dele existe. O único caso em que algo volta é o de um artigo APAGADO
// de vez — por isso, para tirar um artigo da Academia, use "ocultar" no painel
// em vez de excluir.
export async function sincronizarCatalogo() {
  ...
  if (!novos.length) return { inseridos: 0, total: jaTem.size }

  const { data, error } = await supabaseAdmin.from('academia_artigos').insert(novos).select()
  if (error) throw error
  return { inseridos: data?.length || 0, total: jaTem.size + (data?.length || 0) }
}
